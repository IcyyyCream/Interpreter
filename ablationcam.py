...
def gradcam_explain(image_path):
    ...
    cam = AblationCAM(model=model, target_layers=target_layers, use_cuda=torch.cuda.is_available(), batch_size=8)
    # 目标类序号
    # target_label = 0
    # None表示输出值最大类
...
def main(image_num=50):
    ...
    gradcam_save_path = img_save_path + 'ablation_cam/'
    result_save_path = res_save_path + '/ablation_cam/'
