...
from fd_cam.fd_cam import FDCAM
...
def gradcam_explain(image_path):
    ...
    target_category = model(img_tensor).topk(1)[1][0][0].item()
    cam = FDCAM(model, target_layers, 0.95, use_cuda=torch.cuda.is_available())
    # 目标类序号
    # target_label = 0
    # None表示输出值最大类
...
def main(image_num=50):
    ...
    gradcam_save_path = img_save_path + 'fd_cam/'
    result_save_path = res_save_path + '/fd_cam/'
